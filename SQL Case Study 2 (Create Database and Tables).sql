...
left join
FACT_TRANSACTIONS as tbl2 on tbl1.IDLocation = tbl2.IDLocation
...
from FACT_TRANSACTIONS as tbl1
inner join
DIM_LOCATION as tbl2 on tbl1.IDLocation = tbl2.IDLocation
inner join
DIM_MODEL as tbl3 on tbl1.IDModel = tbl3.IDModel
 inner join
 DIM_MANUFACTURER as tbl4 on  tbl3.IDManufacturer = tbl4.IDManufacturer
 group by
 ...
 having
  ...
  from FACT_TRANSACTIONS as tbl1
  left join
  DIM_MODEL as tbl2 on tbl1.IDModel = tbl2.IDModel
  inner join
  DIM_MANUFACTURER as tbl3 on tbl2.IDManufacturer = tbl3.IDManufacturer
  group by
   Manufacturer_Name,Model_Name
   ...
   from FACT_TRANSACTIONS as tbl1
   inner join
   DIM_CUSTOMER as tbl2 on tbl1.IDCustomer = tbl2.IDCustomer
   where DATEPART(YEAR, CONVERT(date,tbl1.Date,105)) in ('2009')
...
order by sum(Quantity) desc) as tbl5)
  ...
  tbl2.IDModel, tbl3.IDManufacturer,Manufacturer_Name, DATEPART(Year,date) as date_year, sum(Quantity) as total_orders
  from FACT_TRANSACTIONS as tbl2 inner join DIM_MODEL as tbl3
  on tbl2.IDModel = tbl3.IDModel inner join DIM_MANUFACTURER as tbl4
  on tbl3.IDManufacturer = tbl4.IDManufacturer 

where DATEPART(year,Date) in ('2009','2010')
group by tbl2.IDModel, DATEPART(year,Date), tbl3.IDManufacturer, Manufacturer_Name
) as tbl1
where rnum = 2
  ...
  tbl2.Manufacturer_Name
  from FACT_TRANSACTIONS as tbl1
  inner join
  DIM_MODEL as tbl3 on tbl1.IDModel = tbl3.IDModel
  left join
  DIM_MANUFACTURER as tbl2 on tbl3.IDManufacturer = tbl2.IDManufacturer
  where
  YEAR(Date) = 2010
  group by
  tbl2.IDManufacturer, tbl2.Manufacturer_Name

  Except

  select tbl2.IDManufacturer, tbl2.Manufacturer_Name from FACT_TRANSACTIONS as tbl1
  inner join 
  DIM_MODEL as tbl3 on tbl1.IDModel = tbl3.IDModel
  left join
  DIM_MANUFACTURER as tbl2 on tbl3.IDManufacturer = tbl2.IDManufacturer
...
SELECT TOP 100 *, 
CONCAT(CAST
(ROUND(COALESCE(((TABLE1.AVG_TRANSACTIONS -(LAG(TABLE1.AVG_TRANSACTIONS,1) OVER (PARTITION BY IDCustomer ORDER BY YEAR_OF_TRANSACTION)))*1.0/TABLE1.AVG_TRANSACTIONS),0)*100.00,2) 
AS FLOAT) , '%')
AS YOY_CHANGE  
FROM  
...
DATEPART(YEAR,Date) AS YEAR_OF_TRANSACTION 
